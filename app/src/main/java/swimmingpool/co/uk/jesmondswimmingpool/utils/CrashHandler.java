...
import swimmingpool.co.uk.jesmondswimmingpool.entity.custom.ErrorBean;
...
public class CrashHandler implements UncaughtExceptionHandler {
    ...
    private boolean handleException(Throwable ex) {
        ...
        new Thread() {
            ...
            public void run() {
                Looper.prepare();
                Toast.makeText(mContext, "sorry, unexpected error happened", Toast.LENGTH_SHORT).show();
                Looper.loop();
            }
        }.start();
        //保存日志文件
    ...
    private String saveCatchInfo2File(Throwable ex) {
        ...
        errorBean.setErrorDate(new Date());
        errorBean.setErrorInfo(sb.toString());
        HttpHelper.getInstance().post(UrlConstant.POST_ERROR, errorBean, new HttpCallBack<CommonEntity<Object>>() {
